...
- [ ] **Config centralization: plumbing-side follow-ups** (2026-07-09, s21 shipped the config modules) — deliberately left OUT of `infra/lambda/shared/config.ts` / `frontend/src/productConfig.ts` (plumbing, not product), but candidates if they ever need tuning: notice `resetAfterDays`/priority values inside `utils/notices.ts`, med custom-interval input clamp in Dashboard.tsx, MIME allow-lists, S3/API CORS maxAge, APNs protocol details, Bedrock `max_tokens`. Also: meds.json/settings.json whole-file PUTs are still last-write-wins (daily.json/household.json got ETag-guarded writes — adopt the same pattern when next touched).
...
- [ ] **Retention ideas parked from the s19 brainstorm** (2026-07-08) — shipped: weight log + weekly digest + **web push (s19 too)**. Still open: (a) **.ics "add to calendar" links** in reminder emails (trivial, high leverage); (b) records health-check line in reminder emails ("Bordetella has no expiry date"). Deliberately rejected: gamification/streaks, photo feeds, more email frequency. **Family pet merge: killed entirely per Mark 2026-07-08** ("disregard completely") — removed from the roadmap board too.
...
- [ ] **First-scan overdue nag for MEDS** (2026-07-09) — s21 shipped the doc side: a record uploaded already-overdue fires a one-time reminder on the first ReminderFn scan after the S3 object appears (LastModified age < 24h), instead of waiting up to ~30 days for the next taper tick. Meds can't use the same trick — `meds.json` is one whole-list file, so any edit refreshes its LastModified and would re-nag every overdue med. Fix = per-med `createdAt` stamped by the PUT validator when an id is first seen, then the same age-<24h rule per med. Small; do it next time the meds route is touched (same visit could adopt `putJsonGuarded` for meds.json, see Family v2 item d).
...
- [x] **Daily history: retain deleted items** (2026-07-09, DONE 2026-07-10) — solved with tombstones instead of archive snapshots (Mark hit it on his phone: deleting a list item erased it from history too). `DailyItem` gained `addedOn`/`removedOn` (stamped by PUT items with the client's local day); removed items stay in `daily.json` invisibly and every read (live window AND archive path) filters by per-date visibility — so past days keep the list they had, and an item added today no longer retro-appears in history. Tombstones live in `items` forever (tiny; compact someday if a list is edited daily for years). REMAINING drift, deliberately unfixed: med rows on past days still derive from the CURRENT meds.json — a deleted med's old check-offs don't display (meds have "Stop tracking" as the keep-history path); renames still show the new name for old days.
