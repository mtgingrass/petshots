...
- [ ] **Daily history: archive item names** (2026-07-09) — s26 made past days browsable (swipe/date dropdown, free 2 weeks / paid 1 year via `daily-archive/`). Archived days store only checks + mood; rows render with CURRENT item/med names, so a check on a since-deleted custom item silently doesn't display, and renamed items show their new name for old days. Fix = snapshot `{id → name}` into the archive (or the day's log) at prune time in `pruneDailyToArchive`. Cosmetic until people edit their lists a lot; do it when the "health trends" reports start reading the archive.
