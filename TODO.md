...
- [ ] **AI extraction: visit-summary smoke fixture** (2026-07-07) — s16 fixed shared service-date extraction (one "Service Date" header + undated service lines, the Greenville Humane Society layout) via a prompt rule, verified against Mark's real cert photo, but `smoke-ai.mjs` fixtures all use per-line dates — that's why the gap slipped through. Add a `makeVisitSummaryCert` fixture to `scripts/lib-cert-pdf.mjs` asserting every line gets dateGiven + suggestedExpiry.
